...
class TopPt: public BaseSelector {
public:
  explicit TopPt(const ParameterSet& config, const TH1* skimCounters);
  virtual ~TopPt() {}
...
TopPt::TopPt(const ParameterSet& config, const TH1* skimCounters)
...
void TopPt::book(TDirectory *dir) {
...
void TopPt::setupBranches(BranchManager& branchManager) {
  fEvent.setupBranches(branchManager);
}

void TopPt::process(Long64_t entry) {
